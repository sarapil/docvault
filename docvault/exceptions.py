...
"""
DocVault — Custom Exception Hierarchy
"""
import frappe


class DocVaultError(Exception):
    """DocVault base exception."""
    def __init__(self, message=None, title=None):
        self.message = message or frappe._("An error occurred in DocVault")
        self.title = title or frappe._("DocVault Error")
...
class NotFoundError(DocVaultError):
...
class PermissionError(DocVaultError):
...
class IntegrationError(DocVaultError):
