...
```bash
bench get-app docvault /path/to/your/fork
bench --site dev.localhost install-app docvault
...
bench --site dev.localhost run-tests --app docvault

# Run specific test — تشغيل اختبار محدد
bench --site dev.localhost run-tests --app docvault --module docvault.tests.unit.test_my_service
```
...
- Update help files in `docvault/help/` for DocType changes
